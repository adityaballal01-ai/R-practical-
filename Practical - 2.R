...
sales_data[sales_data$City=="Banglore" & sales_data$PaymentMethod=="UPI", ]
...
# 5.	Sort by multiple columns.
sales_data[order(sales_data$CustomerName, sales_data$Price ), ]
...
# 1. If discount > 10 mark "High Discount".
sales_data$Discount_Status <- ifelse(sales_data$Discount>10, "high",
                                     ifelse(sales_data$Discount<10,"medium", "low"))
sales_data



# 2. Classify orders into High / Medium / Low value by using if else.
sales_data$Value_Status <- ifelse(sales_data$FinalAmount > 30000, "High Value",
...
# 5.	High price + high quantity
sales_data[sales_data$Price >6000  & sales_data$Quantity > 3, ]
